...
def run_teaser_registration(config: str,
                            source, target, corrs_S, corrs_T):
    ...
    if len(S_corr.transpose()) > 1:
        ...
        print("Scale: ", solution.scale)
        print("Rotation: ", solution.rotation)
        print("Translation: ", solution.translation)

        registration_scale = np.eye(3)*solution.scale
        registration_solution = np.eye(4)
        registration_solution[0:3, 0:3] = solution.rotation @ registration_scale
